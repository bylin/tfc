...
```{r lymph}
metadata <- luad_clinical[luad_clinical$stage %in% c("N0", "N1", "N2", "N3"), ]
counts <- luad_adjusted_counts[, colnames(luad_adjusted_counts) %in% metadata$barcode]
metadata$n_stage <- as.factor(ifelse(metadata$stage %in% c("N0"), "N0" , "N1+"))
lymph_spread <- buildTestCompareGlms(metadata = metadata, covariate = "stage", counts = counts, ntrials = ntrials)
```
...
tsrna_coefs <- tsrna_coefs[tsrna_coefs > 1]
