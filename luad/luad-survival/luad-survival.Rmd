...
metadata = luad_metadata[match(colnames(luad_adjusted_counts), luad_metadata$barcode), ]
```
...
km_metadata = na.omit(metadata[metadata$sample_type != "NT" & !is.na(metadata$days_survived), ]) # get rid of NAs, remove normal samples (e.g. duplicates), and put the time period as 1 month - 8 years
...
metadata = metadata[metadata$sample_type == "TP" & !is.na(metadata$days_survived) & metadata$days_survived > 0, ]
counts = luad_adjusted_counts[rownames(luad_adjusted_counts) %in% c(unique(srnas[srnas$class %in% c("tRF-5", "tRF-3", "tRF-1", "tRF-i")]$tx_name), unique(srnas[str_detect(srnas$class, "mi")]$tx_name)), colnames(luad_adjusted_counts) %in% metadata$barcode]
