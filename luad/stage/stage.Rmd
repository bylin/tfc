...
plotGlms <- function(roc_df) ggplot(roc_df) + geom_line(aes(x = FPR, y = TPR, color = Trial, group = paste0(Trial, Class)), alpha = 0.3) + geom_abline(intercept = 0, slope = 1, colour = "gray") + ylab("TPR") + xlab("FPR") + facet_wrap(~ Class, nrow = 2) + guides(color = FALSE)
...
plotMeanAucs <- function(all_feature_glms) {
  ...
  df <- data.frame(AUC = mean_aucs, Class = as.factor(c("tsrna", "mirna", "snorna", "pirna", "half", "trna")))
...
sample_type <- buildTestCompareGlms(metadata = metadata, covariate = "sample_type", counts = luad_adjusted_counts, ntrials = 100)
```
...
```{r sample-type-auc-plot, fig.width=6, fig.height=4}
...
metadata$metastasis <- as.factor(ifelse(metadata$m_stage == "M0", "Metastasis" , "No metastasis"))
...
```{r metastasis-auc-plot, fig.width=6, fig.height=4}
plotMeanAucs(metastasis$glms)
```

```{r stage-coarse, eval=FALSE}
metadata <- luad_clinical[luad_clinical$m_stage %in% c("M0", "M1", "M1a", "M1b"), ]
counts <- luad_adjusted_counts[, colnames(luad_adjusted_counts) %in% metadata$barcode]
metadata$metastasis <- as.factor(ifelse(metadata$m_stage == "M0", "Metastasis" , "No metastasis"))
metastasis <- buildTestCompareGlms(metadata = metadata, covariate = "metastasis", counts = counts, ntrials = ntrials)
...
```{r stage-plot, fig.width=20, fig.height=8, eval=FALSE}
metastasis$plot
