...
plotGlms <- function(roc_df) ggplot(roc_df) + geom_line(aes(x = FPR, y = TPR, color = Trial), alpha = 0.3) + geom_abline(intercept = 0, slope = 1, colour = "gray") + ylab("TPR") + xlab("FPR") + facet_wrap(~ Class, ncol = 4) + guides(color = FALSE)
...
metastasis <- buildTestCompareGlms(metadata = luad_clinical, covariate = "metastasis", counts = luad_adjusted_counts, srnas = srnas, ntrials = 5)
```
