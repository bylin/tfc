...
lapply(names(metadatas), function(cancer_code) {
  metadata <- metadatas[[cancer_code]]
  sapply(metadata$analysis_id, function(analysis_id) {
    system(paste0('gtdownload -c $CGKEY -d ', analysis_id))
  })
  file.symlink(paste0(getwd(), metadata$analysis_id, '.bam'), paste0('/inside/grotto/blin/trna-markers/datasets/', cancer_code, '/', metadata$shortnames))
})
setwd(currentwd)
```
