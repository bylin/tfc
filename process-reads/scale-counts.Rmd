...
scaleCounts <- function(counts, metadata) {
  ...
  counts <- counts[apply(counts, 1, function(row) sum(as.numeric(row)) > 0), ]
...
readDistribution = function(scaled_counts, adjusted_counts) {
  df = data.frame(counts = as.vector(scaled_counts), type = "Scaled")
  df = rbind(df, data.frame(counts = as.vector(adjusted_counts), type = "Adjusted"))
...
```{r save-data}
save(file = 'luad-counts.RData', luad_adjusted_counts, luad_metadata)
```
