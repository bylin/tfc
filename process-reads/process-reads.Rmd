...
```{r normalization-ma, fig.width=8, fig.height=3, fig.show='hold'}
plotMA <- function(raw_counts, normalized_counts, metadata) {
  ...
  df <- rbind(data.frame(log2FC = log2(raw_tp_counts) - log2(raw_nt_counts), counts = log2((raw_tp_counts + raw_nt_counts)/2), normalized = "Raw"),
              data.frame(log2FC = log2(tp_counts) - log2(nt_counts), counts = log2((tp_counts + nt_counts)/2), normalized = "Normalized"))
  ggplot(df) + geom_point(aes(x = counts, y = log2FC)) + geom_hline(yintercept = 0, color = "red") + facet_wrap(~ normalized)
}
plotMA(luad_raw_counts, luad_normalized_counts, luad_metadata) + ggtitle("LUAD MA plot")
plotMA(brca_raw_counts, brca_normalized_counts, brca_metadata) + ggtitle("BRCA MA plot")
plotMA(prad_raw_counts, prad_normalized_counts, prad_metadata) + ggtitle("PRAD MA plot")
```
...
```{r pca, fig.show="hold", fig.width=12, fig.height=5}
plotPCA <- function(normalized_counts, adjusted_counts, metadata) {
  ...
  df <- rbind(data.frame(PC1 = pca_before$x[, 1], PC2 = pca_before$x[, 2], adjusted = "Adjusted"), data.frame(PC1 = pca_after$x[, 1], PC2 = pca_after$x[, 2], adjusted = "Not adjusted"))
  ...
  ggplot(df) + geom_point(aes(x = PC1, y = PC2, color = batch, shape = batch, pch = sample_type), alpha = 0.5) + guides(color = guide_legend(ncol = 2), shape = guide_legend(ncol = 2), size = guide_legend(ncol = 2)) + facet_wrap(~ adjusted)
}
plotPCA(luad_normalized_counts, luad_adjusted_counts, luad_clinical)
plotPCA(brca_normalized_counts, brca_adjusted_counts, brca_clinical)
plotPCA(prad_normalized_counts, prad_adjusted_counts, prad_clinical)
```
...
save.session("process-reads-image.RData")
```
