...
- v3: Hepatocellular carcinoma (LIHC), colorectal (COAD and READ), pancreatic (PAAD), testicular germ cell (TGCT), acute myeloid leukemia (AML and LAML), lower grade glioma (LGG), glioblastoma (GBM), osteosarcoma (OS).
...
```{r download-files}
metadatas <- list(luad = luad_metadata, brca = brca_metadata, prad = prad_metadata, tgct = tgct_metadata, aml = aml_metadata, lgg = lgg_metadata, gbm = gbm_metadata, os = os_metadata)
currentwd <- getwd()
...
remapHg19 /inside/grotto/blin/trna-markers/datasets/luad
remapHg19 /inside/grotto/blin/trna-markers/datasets/brca
remapHg19 /inside/grotto/blin/trna-markers/datasets/prad
remapHg19 /inside/grotto/blin/trna-markers/datasets/lihc
remapHg19 /inside/grotto/blin/trna-markers/datasets/coad
remapHg19 /inside/grotto/blin/trna-markers/datasets/paad
remapHg19 /inside/grotto/blin/trna-markers/datasets/tgct
remapHg19 /inside/grotto/blin/trna-markers/datasets/aml
remapHg19 /inside/grotto/blin/trna-markers/datasets/lgg
remapHg19 /inside/grotto/blin/trna-markers/datasets/gbm
remapHg19 /inside/grotto/blin/trna-markers/datasets/os
```
...
countFeaturesFromBam <- function(metadata, srnas, dir = ".") {
  ...
  metadata <- metadata[which(file.exists(paste0(dir, "/", metadata$shortnames, "-mature.bam"))), ]
  ...
  counts <- mclapply(1:nrow(metadata), function(i) {
    ...
    # hg19 counts
    features <- srnas[!(srnas$class %in% c("3half", "5half"))]
    bamfile <- paste0(dir, "/", row$shortnames, "-mapped.bam")
    sample <- readGAlignments(bamfile)
    sample <- convertChrNames(sample, "Ensembl")
    sample_counts <- data.frame(countOverlaps(features, sample, minoverlap = 15))
    ...
    sample <- readGAlignments(bamfile)
    sample <- convertChrNames(sample, "Ensembl")
    sample_counts2 <- data.frame(countOverlaps(features, sample, minoverlap = 15))

    sample_counts <- rbind(sample_counts, sample_counts2)
    sample_counts
  }, mc.cores = 20)
  names <- unlist(lapply(counts, colnames))
  counts <- as.data.frame(matrix(unlist(counts), ncol = nrow(metadata)))
  rownames(counts) <- c(srnas[!(srnas$class %in% c("3half", "5half"))]$tx_name, srnas[srnas$class %in% c("3half", "5half")])
