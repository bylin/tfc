...
brca_raw_counts <- countFeaturesFromBam(prad_metadata, features, '/inside/grotto/blin/trna-markers/datasets/prad/')
...
plotMA <- function(raw_counts, normalized_counts, metadata) {
  df <- data.frame(log2FC = log(apply(raw_counts[, metadata[metadata$sample_type == "TP", ]$barcode], 1, mean), 2) - log(apply(raw_counts[, metadata[metadata$sample_type == "NT", ]$barcode], 1, mean), 2),
                   counts = (apply(raw_counts[, metadata[metadata$sample_type == "TP", ]$barcode], 1, mean) + apply(raw_counts[, metadata[metadata$sample_type == "NT", ]$barcode], 1, mean))/2,
                   normalized = FALSE)
  df <- rbind(df, data.frame(log2FC = log(apply(normalized_counts[, metadata[metadata$sample_type == "TP", ]$barcode], 1, mean), 2) - log(apply(normalized_counts[, metadata[metadata$sample_type == "NT", ]$barcode], 1, mean), 2),
                   counts = (apply(normalized_counts[, metadata[metadata$sample_type == "TP", ]$barcode], 1, mean) + apply(normalized_counts[, metadata[metadata$sample_type == "NT", ]$barcode], 1, mean))/2,
                   normalized = TRUE))
  ggplot(df) + geom_point(x = counts, y = log2FC) + geom_hline(yintercept = 0, color = "red") + facet_wrap(~ normalized)
}
...
prad_adjusted_counts <- removeBatchEffects(prad_metadata, prad_normalized_counts)
```
