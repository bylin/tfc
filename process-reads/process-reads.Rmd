...
countFeaturesFromBam <- function(metadata, features, dir = ".") {
  ...
  counts <- mclapply(1:nrow(metadata), function(i) {
    ...
    bamfiles <- paste0(dir, "/", row$shortnames, c("-mapped.bam", "-mature.bam"))
    sample <- readGAlignments(bamfiles)
...
countFeatures <- function(metadata, features, dir = ".") {
  ...
  hg19_counts <- countFeaturesFromBam(metadata, srnas[srnas$class %in% c("3half", "5half")], dir)
  metadata <- metadata[which(file.exists(paste0(dir, "/", metadata$shortnames, "-mapped.bam"))), ])
  mature_counts <- 
}
```
